...
def string(x, n):
	...
	s, e = ''.join(( '{:.', str(n - 2), 'e}')).format(x).split('e')    #JD:The 2 was originally 1
	...
	if e < 0:
		...
		sep = ''
		if i < n: sep = '.'
		if s[0] == '-': i += 1    #Where "==" is the original file had "is" but when I used it an error would occur
...
def scientific(x, n):
	...
	x = float(x)
	if n < 1: raise ValueError("1+ significant digits required.")
	return ''.join(('{:.', str(n - 2), 'E}')).format(x)    #JD: The two was originally a 1
