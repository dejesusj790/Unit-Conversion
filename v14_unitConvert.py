...
#Ideas:
#   1. VideoWindow class, embedding a video about unit conversion for people to watch
...
class PopUp:
    def __init__(self):
        self.window2=Toplevel() #Toplvel() is all you need, it needs no arguements
...
#         self.window3.title("Unit Conversion Video")
#         self.vid_source=vid_source
#         # Opening the video source
#         self.vid=cv2.VideoCapture(self.vid_source)
#         if not self.vid.isOpen(): #In case the video doesn't load
#             raise ValueError("Unable to open vid source,",vid_source)
#         self.vidWidth = self.vid.get(cv2.CAP_PROP_FRAME_WIDTH)
#         self.vidHeight = self.vid.get(cv2.CAP_PROP_FRAME_HEIGHT)
#         self.window3.geometry((str(self.vidWidth)+"x"+str(self.vidHeight)))
#     def __del__(self):
#         if self.vid.isOpen():
#             self.vid.release()
#         self.window3.mainloop()
#     def get_frame(self):    #Gets a frame from the video source
#         if self.vid.isOpen():
...
class MainWindow:
    def __init__(self, window): #__init__ is always ran
        ...
        # self.video_button=Button(window,text="Unit Conversion Video",bg="#76FB28",width=15, command=VideoWindow).grid(row=7,column=0, sticky=W)  
        self.about_button=Button(window,text="About",bg="#FD74B2",width=15, command=self.popup_credit).grid(row=8,column=0, sticky=W)
        ...
        self.user_entry.grid(row=11, column=0, sticky=W)
        self.answer_button=Button(window,text="Enter",bg="#FD74B2",width=5, command=self.check).grid(row=12,column=0, sticky=W)
        self.exit_button=Button(window,text="Exit",bg="#FD74B2",width=5,command=self.quit).grid(row=12,column=0, sticky=E)
